...
def call_random(i, x, training_data, testing_data, fold, goal="Max", eval_time=3600, lifes=5):
    # test_data = testing_data.values
    random_optimizer = Random(NP=10, Goal=goal, eval_time=eval_time, num_lifes=lifes)
    v, num_evals, tuning_time = random_optimizer.solve(process, OrderedDict(param_grid[i]['learners_para_dic']),
...
def run_Random(train, test, perf_measures, learners, name='', repeats = 1, eval_time=3600, lifes=5):
    """
    ...
    for x in perf_measures:
        temp = {}
        save_pickle_address = 'dump_2/%s_random_results/' % x
        ...
        for i in learners:
            ...
            for r in xrange(repeats):
                print("Repeating: %s" % r)
                if x == "d2h":
                    val, params, num_evals, tuning_time = call_random(i, x, train_df, test_df, fold, "Min", eval_time=eval_time,
                                                                  lifes=lifes)
                else:
                    val, params, num_evals, tuning_time = call_random(i, x, train_df, test_df, fold, "Max", eval_time=eval_time,
                                                                  lifes=lifes)
                l.append(val)
                l1.append(params)
                l2.append(num_evals)
...
if __name__ == '__main__':
    ...
    for dataset, datasets in data.items():
        for i in range(len(datasets) - 1):
            run_Random([datasets[i]], [datasets[i + 1]], perf_measures,
