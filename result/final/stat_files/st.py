...
def rdivDemo(data):
    ...
    for _, __, x in sorted(ranks, reverse=True):
        ...
        ret[x.name.split('_')[0]] = max_rank - x.rank + 1
    return ret
    ...
    for line in data:
        for word in line.split():
            ...
            if isinstance(word, str):
                ...
                now += [word]
    data = [[k] + v for k, v in all.items()]
    print(data)
    return rdivDemo(data)


def ranking_stats(ranks, results):
    if ranks['default'] == 1:
        results['default'] += 1
    else:
        for r in ranks:
...
if args.demo:
    ...
    collect = {}
    data = {"ant": ["ant-1.3.csv", "ant-1.4.csv", "ant-1.5.csv", "ant-1.6.csv"],
            "ivy": ["ivy-1.1.csv", "ivy-1.4.csv"],
            "lucene": ["lucene-2.0.csv", "lucene-2.2.csv"],
            "poi": ["poi-1.5.csv", "poi-2.0.csv", "poi-2.5.csv"],
            "synapse": ["synapse-1.0.csv", "synapse-1.1.csv"],
            "velocity": ["velocity-1.4.csv", "velocity-1.5.csv"],
            "camel": ["camel-1.0.csv", "camel-1.2.csv", "camel-1.4.csv"],
            "jedit": ["jedit-3.2.csv", "jedit-4.0.csv", "jedit-4.1.csv", "jedit-4.2.csv"],
            "log4j": ["log4j-1.0.csv", "log4j-1.1.csv"],
            "xerces": ["xerces-1.1.csv", "xerces-1.2.csv", "xerces-1.3.csv"]
            }
    import pickle
    ...
    for perf_measure in perf_measures:
        ...
        for eval in evals:
            collect[perf_measure][eval] = {}
            for l in learners:
                collect[perf_measure][eval][l] = {}
                for data, datasets in data.items():
                    collect[perf_measure][eval][l][data] = {}
                    for o in optimizers:
                        collect[perf_measure][eval][l][data][o] = 0
                    for i in range(len(datasets) - 1):
                        filename = l + "/" + l + "_" + perf_measure + "_" + eval + "_" + datasets[i]
                        temp = another_main(filename)
                        if temp['default'] == 1:
                            collect[perf_measure][eval][l]['default'] += 1
                        else:
                            for k, v in temp.keys():
                                if v == 1:
                                    collect[perf_measure][eval][l][k] += 1

    with open("collect_v1.p", 'wb') as handle:
