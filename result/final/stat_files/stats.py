...
if args.demo:
    ...
    for eval in evals:
        ...
        for perf_measure in perf_measures:
            ...
            for l in learners:
                collect[eval][perf_measure][l] = {}
                for k, v in data.items():
                    collect[eval][perf_measure][l][k] = {}
                    for optimizer in optimizers:
                        collect[eval][perf_measure][l][k][optimizer] = 0
                    for dataset in v:
                        filename = ""
                        filename = l + "/" + l + "_" + perf_measure + "_" + eval + "_" + dataset
                        temp = another_main(filename)
                        if temp['default'] == 1:
                            collect[eval][perf_measure][l][k]['default'] += 1
                        else:
                            for optimizer, rank in temp.items():
                                if rank == 1:
                                    collect[eval][perf_measure][l][k][optimizer] += 1
                    print(collect)
