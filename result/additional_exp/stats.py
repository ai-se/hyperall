...
if args.demo:
    ...
    learners = ['DTC', 'RF', 'SVM', 'KNN']
    optimizers = ['default', 'de', 'random', 'smac', 'grid']
    data = {"ant": ["ant-1.4.csv", "ant-1.5.csv", "ant-1.6.csv", "ant-1.7.csv"],
            "ivy": ["ivy-1.4.csv", "ivy-2.0.csv"],
            "lucene": ["lucene-2.2.csv", "lucene-2.4.csv"],
            "poi": ["poi-2.0.csv", "poi-2.5.csv", "poi-3.0.csv"],
            "synapse": ["synapse-1.1.csv", "synapse-1.2.csv"],
            "velocity": ["velocity-1.5.csv", "velocity-1.6.csv"],
            "camel": ["camel-1.2.csv", "camel-1.4.csv", "camel-1.6.csv"],
            "jedit": ["jedit-4.0.csv", "jedit-4.1.csv", "jedit-4.2.csv", "jedit-4.3.csv"],
            "log4j": ["log4j-1.1.csv", "log4j-1.2.csv"],
    ...
    collect = concatenate_results_per_exp_types(data, perf_measures)

    '''
    for l in learners:
        collect[l] = {}
        for k, v in data.items():
            collect[l][k] = {"default": 0, "de": 0, "random": 0, "smac": 0, "grid": 0}
                    ...
                    for key, rank in temp_results.items():
                        if perf_m != "time":
                            if rank == 1:
                                optimizer = key.split("_")[0]
                                learner = key.split("_")[1]
                                temp[perf_m].add(optimizer)
                                print(perf_m, optimizer)
                        else:
                            list(temp_results).sort(key=lambda x: x[1])
                            temp = temp_results
                same_optimizers = set.intersection(temp["precision"], temp["f1"])

                if not same_optimizers:
                    collect[l][k]["default"] += 1
                else:
                    for j in same_optimizers:
                        collect[l][k][j] += 1
                print(collect[l][k])
    ...
    with open("collect_v6.1.p", 'wb') as handle:
        pickle.dump(collect, handle)
    '''
    ...
    for perf_measure in perf_measures:
        collect[perf_measure] = {}
        for k, v in data.items():
            collect[perf_measure][k] = {}
            for dataset in v:
                collect[perf_measure][k][dataset] = {}
                for l in learners:
                    collect[perf_measure][k][dataset][l] = {}
                    for optimizer in optimizers:
                        collect[perf_measure][k][dataset][l][optimizer] = 0
                        filename = "stats_files/" + perf_measure + "_" + dataset
                        temp = another_main(filename)
                        for optimizer, rank in temp.items():
                            if rank == 1:
                                collect[perf_measure][k][dataset][l][optimizer] += 1
                print(collect)


    with open("collect_v4.p", 'wb') as handle:
        pickle.dump(collect, handle)
    '''