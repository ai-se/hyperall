...
from Constant import MAX_CUTOFF, MAXINT
from utils import sum_cost
...
class Intensifier(object):
    ...
    def __init__(self, tae_runner, stats, traj_logger, rng, instances, instance_specifics=None, cutoff=MAX_CUTOFF,
                 deterministic = False, run_obj_time = True, always_race_against = None, run_limit=MAXINT,
                  minR=1, maxR=2000):
    ...
    def intensify(self, challengers, incumbent, run_history, aggregate_func, time_bound=float(MAXINT), log_traj=True):
        ...
        *Side effect:* adds runs to run_history
        ...
        for challenger in challengers:
            if challenger == incumbent:
                print "Challenger was the same as the current incumbent; Skipping challenger"
                continue

            print "Intensify on %s", challenger
            ...
            if self._chall_indx >= self._min_chall and self._num_run > self.run_limit:
                print "Maximum #runs for intensification reached"
                break
            elif self._chall_indx > 1 and tm - self.start_time - time_bound >= 0:
                print "Timelimit for intensification reached (used: %f sec, available: %f sec)" % (tm - self.start_time, time_bound)
                break

        # output estimated performance of incumbent
        inc_runs = run_history.get_runs_for_config(incumbent)
        inc_perf = aggregate_func(incumbent, run_history, inc_runs)
        print "Updated estimated cost of incumbent on %d runs: %.4f" % (len(inc_runs), inc_perf)
    ...
    def _add_inc_run(self, incumbent, run_history):
    ...
    def _race_challenger(self, challenger, incumbent, run_history, aggregate_func, log_traj=True):
    ...
    def _adapt_cutoff(self, challenger, incumbent, run_history, inc_sum_cost):
        """Adaptive capping:
        ...
        chal_sum_cost = sum_cost(config=challenger,
                                 ...
                                 run_history=run_history)
        cutoff = min(self.cutoff, inc_sum_cost * self.Adaptive_Capping_Slackfactor - chal_sum_cost)
        return cutoff


    def _compare_configs(self, incumbent, challenger, run_history, aggregate_func, log_traj=True):
        """
        ...
        if chal_perf > inc_perf and len(chall_runs) >= self.minR:
            ...
            print "Incumbent (%.4f) is better than challenger (%.4f) on %d runs." % (inc_perf, chal_perf, len(chall_runs))
            return incumbent
        ...
        if not set(inc_runs) - set(chall_runs):
            ...
            if chal_perf >= inc_perf:
                print "Incumbent (%.4f) is at least as good as the challenger (%.4f) on %d runs." % (inc_perf, chal_perf, len(chall_runs))
                return incumbent
            ...
            print "Challenger (%.4f) is better than incumbent (%.4f)  on %d runs." % (chal_perf, inc_perf, n_samples)
            # Show changes in the configuration
            params = sorted([(param, incumbent[param], challenger[param]) for param in challenger.keys()])
            print "Changes in incumbent:"
            for param in params:
                if param[1] != param[2]:
                    print "  %s : %r -> %r" % (param)
                else:
                    print "  %s remains unchanged: %r" % (param[0], param[1])

            if log_traj:
                self.stats.inc_changed += 1
                self.traj_logger.add_entry(train_perf=chal_perf, incumbent_id=self.stats.inc_changed, incumbent=challenger)
            return challenger
